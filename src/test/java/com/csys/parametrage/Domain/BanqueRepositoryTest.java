...
public class BanqueRepositoryTest {
    ...
    @Test
    public void findByCodeShouldReturnBanque() {
        Banque ban = new Banque();
        ban.setDesignation("BIAT");
        ban.setDesignationSec("BIAT");
        ban.setActif(true);
        ban.setUserCreation("user");
        ban.setDateCreation(new Date());
        
        this.repository.save(ban);
        Banque banque = this.repository.findAll().get(0);
        assertThat(banque.getDesignation()).isEqualTo("BIAT");
